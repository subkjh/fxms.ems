...
/**
* para : #vupEngTable, #measrDtmName, $measrDtm, #measrDtmName, $measrDtm, $sinkSource<br>
* ---------------------------------------------------------------------------------- <br>
* database : null<br>
* sql <br><br>
 * <br>insert into #vupEngTable (<br>			#measrDtmName<br>			, INLO_NO<br>			, ENG_ID<br>			, INST_VAL<br>			, PREV_INTG_VAL<br>			, CUR_INTG_VAL<br>			, DIFF_VAL<br>			, PRES_VAL<br>			, TEMP_VAL<br>			, MEMO<br>			, REG_USER_NO<br>			, REG_DTM<br>			, CHG_USER_NO<br>			, CHG_DTM<br>		)<br>		with DATAS as (<br>			select<br>					if(a.PIPE_CL_CD = 'SINK', 'SINK', 'SOURCE') 	as  SINK_SOURCE<br>					, c.MO_NO<br>			from	FE_FAC_PIPE 		a<br>					, FE_FAC_PIPE_PATH 	b<br>					, FX_MO 			c<br>			where	a.PIPE_ID 			= b.PIPE_ID<br>			and 	b.LINK_OBJ_CL_CD 	= 'MO'<br>			and 	c.MO_TID 			= b.LINK_OBJ_ID<br>		)<br>		, DATAS2 as (<br>			select 	 $measrDtm						as #measrDtmName<br>					, a.INLO_NO						as INLO_NO<br>					, a.ENG_ID						as ENG_ID<br>					, max(a.INST_VAL)				as INST_VAL<br>					, max(a.PREV_INTG_VAL)			as PREV_INTG_VAL<br>					, max(a.CUR_INTG_VAL)			as CUR_INTG_VAL<br>					, sum(a.DIFF_VAL)				as DIFF_VAL<br>					, avg(a.PRES_VAL)				as PRES_VAL<br>					, avg(a.TEMP_VAL)				as TEMP_VAL<br>					, 'FE_ENG_MEASR_RAW'			as MEMO<br>					, 1											as REG_USER_NO<br>					, DATE_FORMAT(now(), '%Y%m%d%H%i%s')		as REG_DTM<br>					, 1											as CHG_USER_NO<br>					, DATE_FORMAT(now(), '%Y%m%d%H%i%s')		as CHG_DTM				<br>			from	FE_ENG_MEASR_RAW 	a<br>					, DATAS 			b<br>			where	a.MEASR_DTM		= $measrDtm<br>			and		a.MO_NO 		= b.MO_NO<br>			and		b.SINK_SOURCE	= $sinkSource<br>			group by <br>					a.INLO_NO<br>					, a.ENG_ID<br>		)<br>		select	*<br>		from	DATAS2 	a<br>								<br>		ON DUPLICATE KEY UPDATE		<br>				INST_VAL 		= a.INST_VAL<br>				, PREV_INTG_VAL	= a.PREV_INTG_VAL<br>				, CUR_INTG_VAL	= a.CUR_INTG_VAL<br>				, DIFF_VAL		= a.DIFF_VAL<br>				, PRES_VAL		= a.PRES_VAL<br>				, TEMP_VAL		= a.TEMP_VAL<br>				, MEMO			= a.MEMO<br>				, CHG_DTM		= a.CHG_DTM<br><br> <br>
*/
