 ...
 * . 가동 중 : GasFlowRate > 0<br>
 * . 정지 : 가동 중 상태에 만족하지 않으면서, GasPressure > 0<br>
 * . 데이터 미수신 : 그 외<br>
 * 
 * @author subkjh
 *
...
public class CheckStateGasMeterDfo implements FxDfo<Void, Integer> {

	public static void main(String[] args) {

		CheckStateGasMeterDfo dfo = new CheckStateGasMeterDfo();
		try {
			dfo.call(null, null);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Integer call(FxFact fact, Void empty) throws Exception {
		...
		List<EpwrColMoDataDto> mos = (List<EpwrColMoDataDto>) QidDaoEx
				.SelectDatas(CheckStateSensorDfoQid.QUERY_XML_FILE, QID.select_gas_collected_mo, null);
		...
		List<EpwrColDataDto> datas = (List<EpwrColDataDto>) QidDaoEx.SelectDatas(CheckStateSensorDfoQid.QUERY_XML_FILE,
				QID.select_gas_collected_data, FxApi.makePara("minute", minute * dataSize));
