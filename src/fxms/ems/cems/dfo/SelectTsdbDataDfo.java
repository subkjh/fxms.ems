...
public class SelectTsdbDataDfo implements FxDfo<Void, List<PsVoRawList>> {
	...
	public List<PsVoRawList> selectData(long startDtm, long endDtm) throws Exception {
		...
		for (String measurement : MEASUREMENTS) {
			...
			selectDatas(measurement, sql, values);
	...
	private void selectDatas(String measurement, String sql, Map<Long, PsVoRawList> values) throws Exception {
				...
				if (r.getSeries() != null) {
					for (Series s : r.getSeries()) {
						...
						for (List<Object> list : s.getValues()) {
							...
							if ("busan.172_29_11_100.DX01006".equals(moTid)) {
								try {
									Logger.logger.info(">>>> {}.{} {} {}", moTid, mo, DateUtil.toHstime(time),
											list.get(2));
								} catch (Exception e) {
...
//								}
			...
			Logger.logger.debug("{} time row(s) fetched - {}ms", values.size(), (System.currentTimeMillis() - ptime));
